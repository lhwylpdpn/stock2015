...
def	run(stockid_i,date1,time1,lnA_B,lnA_B_sub,close_1,norm_,avg_,stdev_,sample,commission,norm_num,norm_lncha,norm_temp):
	...
	cangweiall=0
	cangwei=[]
	bucang_zhiying_value=[]
	bucang_open=[]
	bucang_count=[]
	bucang_close_status=[]
	bucang_count_max=[]
	zhisun_shengyucangwei=[]
	bucang_zhisun_value=[]
	zd_c_max_count_max=[]
	zd_c_max_count=[]
	zd_c_max_zhiying_value=[]
	...
	for i in range(len(lnA_B)):
		...
		if norm_[i]=="" or norm_[i-1]=="" or norm_[i-2]=="":
			...
			values.append(0)
			zhisun_shengyucangwei.append(0)
			huiche.append(0)
			continue
		...
		if i>102 and norm_[i]>0.88 and  norm_[i]<0.99 and norm_lncha[i]<0.9 and norm_[i-1]<0.88 and norm_[i-2]<0.88:#and norm_[i]-min(norm_[i-1],norm_[i-2],norm_[i-3])<0.6:
			if i+1<len(lnA_B):
				open_.append(1)
				
				for j in range(i+1,len(lnA_B)):
					
					if lnA_B[j]<scipy.stats.norm.ppf(0.85,avg_[i],stdev_[i]):
						...
						break
					if close_1[j]-close_1[i]>close_1[i]/150:
						close_.append(j)
						bucang_open.append(close_1[j])
						bucang_count.append(1)
						bucang_open.append(close_1[j])
						clsoe_stats.append("补仓开始"+str(j))
						bucang_count.append(1)
						bucang_count_max.append(0)
						for k in range(j+1,len(lnA_B)): 
							if close_1[k]-bucang_open[len(bucang_open)-1]>close_1[i]/150:
								#bucang_zhisun_value.append((close_1[k]-bucang_open[len(bucang_open)-1])*bucang_count[len(bucang_count)-1])
								bucang_open.append(bucang_open[len(bucang_open)-1]+close_1[i]/150)
								bucang_count.append(bucang_count[len(bucang_count)-1]+bucang_count[len(bucang_count)-2])
								bucang_count_max.append((bucang_count[len(bucang_count)-1]+bucang_count[len(bucang_count)-2]))
							if bucang_open[len(bucang_open)-1]-close_1[k]>close_1[i]/150:

								bucang_zhiying_value.append((close_1[i]/150)*bucang_count[len(bucang_count)-1])
								bucang_open.pop()
								bucang_count.pop()
							if len(bucang_count)==1:
								chicang_shijian.append(k-j)
								break



						values.append(sum(bucang_zhiying_value))
						zhisun_shengyucangwei.append(max(bucang_count))
						zhisun_value.append(0)
						zhisun_count=zhisun_count+1
						zhisun_time.append(max(chicang_shijian))
						huiche.append(0)
...
#######################################
				...
				huiche.append(0)
				zhisun_shengyucangwei.append(0)
		elif i>102  and norm_[i]<0.12 and norm_[i]>0.01 and  norm_[i]>0.01 and norm_lncha[i]>0.1 and norm_[i-1]>0.12 and norm_[i-2]>0.12:#  and norm_[i]-max(norm_[i-1],norm_[i-2],norm_[i-3])>-0.6: #and abs(scipy.stats.norm.cdf(lnA_BaddC[i-1],avg_[i],stdev_[i])-scipy.stats.norm.cdf(lnA_BaddC[i],avg_[i],stdev_[i]))>0.5:
			...
			if i+1<len(lnA_B):
				...
				for j in range(i+1,len(lnA_B)):
					...
					if lnA_B[j]>scipy.stats.norm.ppf(0.15,avg_[i],stdev_[i]) :

						close_.append(j)
						clsoe_stats.append("正态交易")
						values.append(close_1[j]-close_1[i])
						zhengtai_value.append(close_1[j]-close_1[i])
						zhengtai_count=zhengtai_count+1
						zhengtai_time.append(j-i)
					...
					if (close_1[j]-close_1[i])/close_1[i]<-0.005:
						...
						values.append(close_1[j]-close_1[i])
						zhisun_value.append(close_1[j]-close_1[i])
						zhisun_count=zhisun_count+1
					...
					if j>=len(lnA_B)-1:
						...
						values.append(-1)
						huiche.append(0)	
						zhisun_shengyucangwei.append(-1)
			else:
				open_.append(0)
				close_.append(0)
				clsoe_stats.append(0)		
	...
	write_result(stockid_i,date1,time1,lnA_B,close_1,norm_,avg_,stdev_,sample,commission,norm_num,open_,close_,clsoe_stats,values,zhisun_shengyucangwei)
def writle_count(word):
	print(str(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))))
	file_object = open('test/count2.csv','a')
	file_object.write(word+"\n")
	file_object.close()
def writle_title():
	file_object = open('test/count2.csv','a')
	file_object.write("\n"+"ida,成功单数,失败单数,成功率,ln振幅,平均成功持仓时间,平均失败持仓时间,平均成功盈利点数,平均失败损失点数,最大回撤率,数学期望,顺势成功单数,顺势失败单数,顺势成功率,平均顺势成功时间,平均顺势失败时间,顺势最大回撤"+"\n")
	file_object.close()
def write_result(stockid_i,date1,time1,lnA_B,close_1,norm_,avg_,stdev_,sample,commission,norm_num,open_,close_,clsoe_stats,values,zhisun_shengyucangwei):
	file_object = open('test/result.csv','w')

	file_object.write("id,stockid_i,date1,time1,lnA_B,close_1,norm_,avg_,stdev_,sample,commission,norm_num,open_,close_,clsoe_stats,values,zhisun_shengyucangwei"+"\n")
	# print(len(stockid_i))
	# print(len(stockid_j))
	# print(len(date1))
	# print(len(time1))
	...
	# print(len(values))
	print(len(lnA_B),len(close_),len(zhisun_shengyucangwei),len(values),len(date1))
	
	for i in range(len(lnA_B)):
		#print(str(huiche[i]))
		file_object.write(str(i)+","+str(stockid_i[i])+","+str(date1[i])+","+str(time1[i])+","+str(lnA_B[i])+","+str(close_1[i])+","+str(norm_[i])+","+str(avg_[i])+","+str(stdev_[i])+","+str(sample)+","+str(commission)+","+str(norm_num)+","+str(open_[i])+","+str(close_[i])+","+str(clsoe_stats[i])+","+str(values[i])+","+str(zhisun_shengyucangwei[i])+"\n")
	file_object.close()
			...
			for r in range(0,len(close_1)):
				...
				if norm_temp[r]!="" and norm_temp[r-1]!="" and norm_temp[r-2]!="":
					if sum(norm_lncha[r-2:r+1])/len(norm_lncha[r-2:r+1])>0.9 or sum(norm_lncha[r-2:r+1])/len(norm_lncha[r-2:r+1])<0.1 :
						...
						i_norm=0
						test_w.append(sum(norm_lncha[r-2:r+1])/len(norm_lncha[r-2:r+1]))
...
def run_writetest(stockid_i,date1,time1,lnA_B,lnA_B_sub,close_1,norm_,avg_,stdev_,sample,commission,norm_num,norm_lncha,norm_temp):
	...
	for i in range(len(lnA_B)):
		#print(str(huiche[i]))
		file_object.write(str(stockid_i[i])+","+str(date1[i])+","+str(time1[i])+","+str(lnA_B[i])+","+str(lnA_B_sub[i])+","+str(close_1[i])+","+str(norm_[i])+","+str(avg_[i])+","+str(stdev_[i])+","+str(sample)+","+str(commission)+","+str(norm_num)+","+str(norm_lncha[i])+","+str(norm_temp[i])+"\n")
	file_object.close()
